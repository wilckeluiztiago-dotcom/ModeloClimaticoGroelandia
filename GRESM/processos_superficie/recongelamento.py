...
class RecongelamentoBase(ABC):
    """Classe base abstrata para Recongelamento."""
    @abstractmethod
    def inicializar(self): pass
    @abstractmethod
    def executar(self, *args, **kwargs): pass

class Recongelamento(RecongelamentoBase):
    """
    Implementação de Recongelamento com lógica de alta fidelidade.
    ...
    def calcular_refreeze(self, agua_liquida, temp_camada):
        """Implementação específica: calcular_refreeze"""
        self._status = "COMPUTING_CALCULAR_REFREEZE"
        try:
            # Validação
            # self._validar_entradas( agua_liquida)
            
            # Kernel Físico/Lógico
            cap = np.maximum(0, -temp_camada * 2097 / 333500); return np.minimum(agua_liquida, cap)
