...
class SmbAblacaoBase(ABC):
    """Classe base abstrata para SmbAblacao."""
    @abstractmethod
    def inicializar(self): pass
    @abstractmethod
    def executar(self, *args, **kwargs): pass

class SmbAblacao(SmbAblacaoBase):
    """
    Implementação de SmbAblacao com lógica de alta fidelidade.
    ...
    def calcular_derretimento(self, temp_ar):
        """Implementação específica: calcular_derretimento"""
        self._status = "COMPUTING_CALCULAR_DERRETIMENTO"
        try:
            # Validação
            # self._validar_entradas( temp_ar)
            
            # Kernel Físico/Lógico
            pdd = np.maximum(temp_ar, 0); return pdd * 0.008 * 100
