...
class RoteadorEscoamentoBase(ABC):
    """Classe base abstrata para RoteadorEscoamento."""
    @abstractmethod
    def inicializar(self): pass
    @abstractmethod
    def executar(self, *args, **kwargs): pass

class RoteadorEscoamento(RoteadorEscoamentoBase):
    """
    Implementação de RoteadorEscoamento com lógica de alta fidelidade.
    ...
    def fluxo_rio(self, runoff_total):
        """Implementação específica: fluxo_rio"""
        self._status = "COMPUTING_FLUXO_RIO"
        try:
            # Validação
            # self._validar_entradas( runoff_total)
            
            # Kernel Físico/Lógico
            return runoff_total * 0.8 # Atraso hidrologico
