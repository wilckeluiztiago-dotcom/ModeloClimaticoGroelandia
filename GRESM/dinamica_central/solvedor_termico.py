...
class SolvedorTermicoBase(ABC):
    """Classe base abstrata para SolvedorTermico."""
    @abstractmethod
    def inicializar(self): pass
    @abstractmethod
    def executar(self, *args, **kwargs): pass

class SolvedorTermico(SolvedorTermicoBase):
    """
    Implementação de SolvedorTermico com lógica de alta fidelidade.
    ...
    def evoluir_temperatura(self, temp_antiga, adveccao, difusao, dt):
        """Implementação específica: evoluir_temperatura"""
        self._status = "COMPUTING_EVOLUIR_TEMPERATURA"
        try:
            # Validação
            # self._validar_entradas( temp_antiga)
            
            # Kernel Físico/Lógico
            return temp_antiga + (difusao - adveccao) * dt
