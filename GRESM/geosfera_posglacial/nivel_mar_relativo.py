...
class NivelMarRelativoBase(ABC):
    """Classe base abstrata para NivelMarRelativo."""
    @abstractmethod
    def inicializar(self): pass
    @abstractmethod
    def executar(self, *args, **kwargs): pass

class NivelMarRelativo(NivelMarRelativoBase):
    """
    Implementação de NivelMarRelativo com lógica de alta fidelidade.
    ...
    def calcular_rsl(self, eustatico, gia, geoide):
        """Implementação específica: calcular_rsl"""
        self._status = "COMPUTING_CALCULAR_RSL"
        try:
            # Validação
            # self._validar_entradas( eustatico)
            
            # Kernel Físico/Lógico
            return eustatico - gia + geoide
