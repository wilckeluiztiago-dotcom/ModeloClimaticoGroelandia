...
class RadiacaoSolarBase(ABC):
    """Classe base abstrata para RadiacaoSolar."""
    @abstractmethod
    def inicializar(self): pass
    @abstractmethod
    def executar(self, *args, **kwargs): pass

class RadiacaoSolar(RadiacaoSolarBase):
    """
    Implementação de RadiacaoSolar com lógica de alta fidelidade.
    ...
    def insolacao_toa(self, dia_ano, lat):
        """Implementação específica: insolacao_toa"""
        self._status = "COMPUTING_INSOLACAO_TOA"
        try:
            # Validação
            # self._validar_entradas( dia_ano)
            
            # Kernel Físico/Lógico
            decl = 23.45 * np.sin(2*np.pi*(284+dia_ano)/365); return 1367 * np.cos(np.deg2rad(lat-decl))
