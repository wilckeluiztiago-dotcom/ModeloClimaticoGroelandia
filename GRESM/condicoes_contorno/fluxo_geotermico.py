...
class FluxoGeotermicoBase(ABC):
    """Classe base abstrata para FluxoGeotermico."""
    @abstractmethod
    def inicializar(self): pass
    @abstractmethod
    def executar(self, *args, **kwargs): pass

class FluxoGeotermico(FluxoGeotermicoBase):
    """
    Implementação de FluxoGeotermico com lógica de alta fidelidade.
    ...
    def heatmap_basal(self, x, y):
        """Implementação específica: heatmap_basal"""
        self._status = "COMPUTING_HEATMAP_BASAL"
        try:
            # Validação
            # self._validar_entradas( x)
            
            # Kernel Físico/Lógico
            return 0.060 + 0.01 * np.exp(-((x-500)**2 + (y-800)**2)/100000)
