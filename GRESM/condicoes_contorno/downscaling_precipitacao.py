"""
Módulo: downscaling_precipitacao.py
...
class DownscalingPrecipitacaoBase(ABC):
    """Classe base abstrata para DownscalingPrecipitacao."""
    @abstractmethod
    def inicializar(self): pass
    @abstractmethod
    def executar(self, *args, **kwargs): pass

class DownscalingPrecipitacao(DownscalingPrecipitacaoBase):
    """
    Implementação de DownscalingPrecipitacao com lógica de alta fidelidade.
    ...
    def aplicar_orografia(self, prec_base, elevacao):
        """Implementação específica: aplicar_orografia"""
        self._status = "COMPUTING_APLICAR_OROGRAFIA"
        try:
            # Validação
            # self._validar_entradas( prec_base)
            
            # Kernel Físico/Lógico
            return prec_base * (1 + elevacao / 2000.0)
