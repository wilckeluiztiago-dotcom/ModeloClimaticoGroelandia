...
class TaxaGradienteTempBase(ABC):
    """Classe base abstrata para TaxaGradienteTemp."""
    @abstractmethod
    def inicializar(self): pass
    @abstractmethod
    def executar(self, *args, **kwargs): pass

class TaxaGradienteTemp(TaxaGradienteTempBase):
    """
    Implementação de TaxaGradienteTemp com lógica de alta fidelidade.
    ...
    def calcular_lapse_rate(self, estacao):
        """Implementação específica: calcular_lapse_rate"""
        self._status = "COMPUTING_CALCULAR_LAPSE_RATE"
        try:
            # Validação
            # self._validar_entradas( estacao)
            
            # Kernel Físico/Lógico
            return -6.5 if estacao == 'verao' else -4.0
